/*
 * GXR Annotate Layer v0.5 — world-anchored transparent annotation layer for GraphXR.
 *
 * Three anchor classes in one layer:
 *   - node-derived: callouts (nodeId + label offset), cluster circles (nodeIds[]),
 *     arrows with node-snapped tips — follow the data through layout moves
 *   - free graph-space marks: arrows/ellipses/text/steps in cloudScene-local coords —
 *     glued through pan/zoom (the app's pan/zoom mutates the cloudScene transform)
 *   - screen-fixed: per-view title/caption
 *
 * Safety (v0.4 pass):
 *   - sidecar clobber guard: refuses to save until a load has succeeded; 404 != error
 *   - merge-safe saves: read-modify-write per view key; .bak written before upload
 *   - race-proof view keys: URL perspective-id watching + key resolved at draw time
 *   - fail-visible: feature detection at install; render-loop errors surface a banner
 *   - export alignment: canvas-only capture, ratio-checked
 *
 * Projection (from graphxr drawing.js): screen = dr.convertCloudPoint(cloudLocalVec3);
 * inverse = unproject NDC via dr.camera, ray into cloudScene-local, intersect local z=0.
 *
 * Singleton: window.__GXR_ANNOTATE__ (re-injection replaces the old instance).
 * NOTE: no backticks anywhere in this file — it is embedded inside a grovebook cell.
 */
(function () {
  var w = window;
