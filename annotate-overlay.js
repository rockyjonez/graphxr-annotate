/*
 * GXR Annotate Layer — world-anchored transparent annotation layer for GraphXR.
 *
 * Runs in the MAIN app window (injected by the bootstrap grovebook). Draws an
 * SVG overlay above the #renderer WebGL canvas; annotations are stored in
 * WORLD coordinates and re-projected every frame through the live THREE
 * camera, so they stay glued to the graph through pan/zoom/rotate.
 *
 * Persistence: sidecar file /annotations.sidecar.json in the project, via
 * gxr.files.upload / gxr.files.get, keyed by saved-view id (drafts keyed by
 * project id). The layer polls gxr.views.getCurrent() and swaps sets when the
 * loaded view changes.
 *
 * Singleton: window.__GXR_ANNOTATE__ (re-injection replaces the old instance).
 * NOTE: no backticks in this file — it is embedded inside a grovebook cell.
 */
(function () {
  var w = window;
